...
const FlipCardGame = () => {
  ...
  const handleCardClick = (char) => {
    if (isRevealing || selected.length === 2 || matched.includes(char)) return;
    ...
    if (newSelection.length === 2) {
      ...
      if (keyMap[first.char] === second.char || keyMap[second.char] === first.char) {
        ...
        setTimeout(() => {
          setCards(cards.filter(c => ![first.char, second.char].includes(c.char)));
          setSelected([]);
        }, 500);
      } else {
        setTimeout(() => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      ...
      <div className="flex gap-4 mb-4">
        ...
        {!isRevealing && (
          <button
            className={`px-4 py-2 rounded-lg shadow-md transition ${hintsLeft > 0 ? "bg-yellow-400 hover:bg-yellow-500" : "bg-gray-400 cursor-not-allowed"
              }`}
            onClick={useHint}
          ...
          {cards.map((item, index) => (
            <div
              ...
              className={`w-16 h-16 flex justify-center items-center text-2xl font-semibold 
                         ...
                         ${isRevealing ? "bg-green-300" : "bg-blue-300 hover:bg-blue-400"} 
                         ${matched.includes(item.char) ? "opacity-0" : ""}`}
              onClick={() => handleCardClick(item)}
            >
              {isRevealing || selected.includes(item) ? (
      ...
      {showMenu && (
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            ...
            <button className="bg-red-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-red-600 transition mr-4" onClick={() => navigate("/")}>
              Exit Game
            </button>
            <button className="bg-gray-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-gray-600 transition" onClick={() => setShowMenu(false)}>
              Close Menu
            </button>
          </div>
        </div>
